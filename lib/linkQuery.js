...
require('./reducer/index.js')
require('./linker/index.js')
...
class Query {
  ...
  async fetch() {
    ...
    this.collection.reducerApply(this._result)
  ...
  _prepareQuery() {
    const linksName = this.collection.linkGetAllNames()
    this._needLinksName = _.intersection(linksName, Object.keys(this.body))
    if (this._needLinksName.length) {
      this._prepareProjection = this._needLinksName.map(v => this.collection.linkGet(v).field)
    }
  }
  ...
  _link() {
    ...
    if (this._needLinksName.length) {
      ...
      this._needLinksName.forEach((linkName) => {
        const config = this.collection.linkGet(linkName)
        switch (config.type) {
        case 'one':
